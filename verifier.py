...
def verify_sudoku(puzzle: str) -> bool:
    ...
    if (
        len(puzzle) != 81
        or not puzzle.isnumeric()
        or not puzzle.replace("0", "a").isnumeric()
    ):
    ...
    for box_index in [
        ...
        57,
        60,
    ]:  # NOTE: box_index = NW corner of 3x3 box
        box = (
            puzzle[box_index : box_index + 3]
            + puzzle[box_index + 9 : box_index + 9 + 3]
            + puzzle[box_index + 18 : box_index + 18 + 3]
        )
...
def _verify_section(chunk: str) -> bool:
    ...
    if len(chunk) != 9:
        raise ValueError(
            f"Did not receive the expected 9 characters, and instead got '{chunk}'"
        )
